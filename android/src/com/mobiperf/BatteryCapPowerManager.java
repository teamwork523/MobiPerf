...
/**
 * A basic power manager implementation that decides whether a measurement can
 * be scheduled based on the current battery level: no measurements will be
 * scheduled if the current battery is lower than a threshold.
 */
public class BatteryCapPowerManager {
	...
	/**
	 * Sets the minimum battery percentage below which measurements cannot be
	 * run.
	 * 
	 * @param batteryThresh
	 *            the battery percentage threshold between 0 and 100
	 */
	public synchronized void setBatteryThresh(int batteryThresh)
			throws IllegalArgumentException {
		if (batteryThresh < 0 || batteryThresh > 100) {
			throw new IllegalArgumentException(
					"batteryCap must fall between 0 and 100, inclusive");
		}
		this.minBatteryThreshold = batteryThresh;
	}
	...
	public synchronized boolean canScheduleExperiment() {
		return (PhoneUtils.getPhoneUtils().isCharging() || PhoneUtils
				.getPhoneUtils().getCurrentBatteryLevel() > minBatteryThreshold);
	}

	/**
	 * A task wrapper that is power aware, the real logic is carried out by
	 * realTask
	 * 
	...
	public static class PowerAwareTask implements Callable<MeasurementResult> {
		...
		public PowerAwareTask(MeasurementTask task,
				BatteryCapPowerManager manager, MeasurementScheduler scheduler) {
			realTask = task;
			pManager = manager;
		...
		private void broadcastMeasurementStart() {
			...
			intent.putExtra(UpdateIntent.STATUS_MSG_PAYLOAD, "Running "
					+ realTask.getDescriptor());
